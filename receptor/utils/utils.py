...
def discount_rewards_onestep(rewards, gamma, expected_values):
    """Applies reward discounting for trajectories.

    Args:
        rewards: List of rewards.
        gamma (float): Discount factor.
        expected_values: List of expected future rewards for each reward.
...
def discount_rewards(rewards, gamma, expected_reward=0.0, terminals=None):
    ...
    for i in reversed(range(len(rewards))):
        discount_sum = rewards[i] + gamma * discount_sum * (1. - terminals[i])
...
def discount_trajectory(rewards, terms, traj_ends, gamma, expected_values):
    ...
    Args:
        ...
        terms (list): List of bools indicating terminal states for each transition.
    ...
    # Bootstrap from every state where trajectory ends
    # Do not boostrap from terminal state (take reward from env)
